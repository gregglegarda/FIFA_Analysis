...
forward <- c("LS", "LW", "LF", "ST", "CF", "RF", "RS", "RW")
back <- c("LWB", "LB", "CB", "LCB", "RCB", "RB", "RWB")
...
```{r}
fifa <- fifa %>% 
  mutate(PositionGroup = ifelse(Position %in% back, "Back", ifelse(Position %in% middle, "Middle", ifelse(Position %in% forward, "Forward", "Unknown"))))
...
PositionValue <- fifa %>%
  ...
  scale_y_log10(labels = dollar_format(prefix = "€")) +
  ggtitle("Player's Value based on Positions", subtitle = "Attackers have more value than players in the midfield and backfield.") +
  theme_economist()
...
```{r}
fifa_aplayers <- na.omit(fifaAttackers[c("Position", skills_names, "Value")])
```
...
``` {r outliers, echo=FALSE, include=TRUE}
# # remove outliers
...
#### HISTOGRAM
We can see below in the histogram the both Wage and Value of the playeds are heavily skewed to the right.
...
#Wage residuals
vif(wage_lm)
wage_lm_res <- resid(wage_lm)
#hist(wage_lm_res, main="Histogram of Residuals for Wage", xlab="Residuals for Wage", ylab="Frequency", col=barcolors)
```

We can see in the VIF results the most correlated skills with Wage:
GK Reflexes
GK Diving
Standing Tackle
GK Handling
...
vif(value_lm)
value_lm_res <- resid(value_lm)
#hist(value_lm_res, main="Histogram of Residuals for Value", xlab="Residuals for Value", ylab="Frequency", col=barcolors)

```
We can see in the VIF results the most correlated skills with Value
GK Reflexes
GK Diving
Standing Tackle
GK Handling
